...
- Create Angular project with ng: `ng new csv_import.client --defaults --skip-install --skip-git --no-standalone `.
...
- Create project file (`csv_import.client.esproj`).
- Create `launch.json` to enable debugging.
- Create `nuget.config` to specify location of the JavaScript Project System SDK (which is used in the first line in `csv_import.client.esproj`).
